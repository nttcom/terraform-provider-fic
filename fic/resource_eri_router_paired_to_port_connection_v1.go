...
func resourceEriRouterPairedToPortConnectionV1() *schema.Resource {
	return &schema.Resource{
		Create: resourceEriRouterPairedToPortConnectionV1Create,
		Read:   resourceEriRouterPairedToPortConnectionV1Read,
		Update: resourceEriRouterPairedToPortConnectionV1Update,
		Delete: resourceEriRouterPairedToPortConnectionV1Delete,
...
func getSourceHAInfoOfRouterPairedToPortConnection(i map[string]interface{}) connections.SourceHAInfo {
...
func getSourceHAInfoOfRouterPairedToPortConnectionForUpdate(i map[string]interface{}) connections.SourceHAInfoForUpdate {
	asPathPrependIn := i["as_path_prepend_in"]
	asPathPrependOut := i["as_path_prepend_out"]
...
func getSourceOfRouterPairedToPortConnection(d *schema.ResourceData) connections.Source {
	tmpSource := d.Get("source_information").([]interface{})

	tmpPrimary := tmpSource[0].(map[string]interface{})
	primary := getSourceHAInfoOfRouterPairedToPortConnection(tmpPrimary)
	...
	secondary := getSourceHAInfoOfRouterPairedToPortConnection(tmpSecondary)
...
func getSourceOfRouterPairedToPortConnectionForUpdate(d *schema.ResourceData) connections.SourceForUpdate {
	tmpSource := d.Get("source_information").([]interface{})

	tmpPrimary := tmpSource[0].(map[string]interface{})
	primary := getSourceHAInfoOfRouterPairedToPortConnectionForUpdate(tmpPrimary)
	source := connections.SourceForUpdate{
		Primary: primary,
	}

	tmpSecondary := tmpSource[1].(map[string]interface{})
	secondary := getSourceHAInfoOfRouterPairedToPortConnectionForUpdate(tmpSecondary)
	source.Secondary = secondary
...
func getDestinationOfRouterPairedToPortConnection(d *schema.ResourceData) connections.Destination {
...
func resourceEriRouterPairedToPortConnectionV1Create(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	...
	createOpts := &connections.CreateOpts{
		...
		Source:      getSourceOfRouterPairedToPortConnection(d),
		Destination: getDestinationOfRouterPairedToPortConnection(d),
	...
	return resourceEriRouterPairedToPortConnectionV1Read(d, meta)
}

func getSourceInformationOfRouterPairedToPortConnectionForState(r *connections.Connection) []map[string]interface{} {
...
func getDestinationOfRouterPairedToPortConnectionInformationForState(r *connections.Connection) []map[string]interface{} {
	primary := map[string]interface{}{
...
func resourceEriRouterPairedToPortConnectionV1Read(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.eriV1Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating FIC ERI client: %s", err)
	...
	d.Set("source_information", getSourceInformationOfRouterPairedToPortConnectionForState(r))
	d.Set("destination_information", getDestinationOfRouterPairedToPortConnectionInformationForState(r))
...
func resourceEriRouterPairedToPortConnectionV1Update(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.eriV1Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating FIC ERI client: %s", err)
	...
	if d.HasChange("source_information") {
		updateOpts := connections.UpdateOpts{
			Source: getSourceOfRouterPairedToPortConnectionForUpdate(d),
		}
...
func resourceEriRouterPairedToPortConnectionV1Delete(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.eriV1Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating FIC ERI client: %s", err)
